...
async function fetchProduct(code) {
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/product?code=${code}`, {
    cache: 'no-store',
  });
  if (!res.ok) {
...
function getAdditionalInfoIcon(type) {
  switch (type) {
...
function getLocationDetails(location) {
  ...
  if (location.name) {
    return {
      title: location.name,
      subtitle: location.address ? [location.address.street, location.address.state, location.address.country].filter(Boolean).join(', ') : undefined,
    };
  }
  switch (location.reference) {
    case 'CONTACT_SUPPLIER_LATER':
    ...
    default:
      if (location.address) {
        return {
          ...
          subtitle: [location.address.street, location.address.state, location.address.country].filter(Boolean).join(', '),
        };
      }
...
export default async function ProductDetails(props) {
  const { params } = props;
  const { code } = await params; // Fix: Await params to resolve the Promise
  let product = null;
  let error = null;
    ...
    <div className="container mx-auto py-8 px-4 max-w-6xl">
      <div className="mb-8">
        ...
        <div className="flex items-center gap-4 mb-6">
          <div className="flex items-center bg-teal-100 px-3 py-1 rounded-full">
            ...
            <span className="text-teal-800 font-medium">{averageRating.toFixed(1)} ({reviewCount} reviews)</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm h-fit sticky top-4">
          ...
          {product.productUrl && (
            <Button asChild className="w-full bg-teal-600 hover:bg-teal-700 text-white font-medium py-3">
              <Link href={product.productUrl} target="_blank" rel="noopener noreferrer">Book Now</Link>
            </Button>
          )}
            ...
            {product.itinerary.days.map((day) => (
              <div key={day.dayNumber} className="border border-gray-200 rounded-lg overflow-hidden">
                <div className="bg-gray-50 p-4 border-b border-gray-200">
                  <h3 className="text-xl font-semibold text-black">Day {day.dayNumber}: {day.title}</h3>
                </div>
                <div className="p-4">
                      ...
                      return (
                        <li key={index} className="flex gap-4">
                          <div className="flex flex-col items-center">
                            ...
                            {index < day.items.length - 1 && <div className="w-0.5 h-full bg-gray-200"></div>}
                          </div>
                          <div className="flex-1 pb-4">
                            <div className="flex items-center gap-2 mb-1">
                              <MapPin className="w-4 h-4 text-teal-600" />
                              <h4 className="font-medium text-black">{locationInfo.title}</h4>
                            </div>
                            <p className="text-black mb-2 font-semibold">{item.description}</p>
                            {locationInfo.subtitle && <p className="text-gray-500 text-sm mb-2">{locationInfo.subtitle}</p>}
                      ...
                      {day.foodAndDrinks?.length > 0 && (
                        <div className="flex items-start gap-2 mb-2">
                          ...
                          <div>
                            ...
                            <p className="text-gray-600">{day.foodAndDrinks.map(fd => fd.type).join(', ')}</p>
                          </div>
                        </div>
                      )}
                      {day.accommodations?.length > 0 && (
                        <div className="flex items-start gap-2">
                          ...
                          <div>
                            ...
                            <p className="text-gray-600">{day.accommodations.map(a => a.description).join(', ')}</p>
                          </div>
                        </div>
                      )}
                    </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-black">{formatTransportationType(product.logistics.travelerPickup.pickupOptionType)}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-teal-100 flex items-center justify-center">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-black">{product.logistics.travelerPickup.minutesBeforeDepartureTimeForPickup} minutes before departure</p>
                  </div>
                </div>
                {product.logistics.travelerPickup.additionalInfo && (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-black">{product.logistics.travelerPickup.additionalInfo}</p>
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <div className="flex items-center mb-1">
                    {[...Array(5)].map((_, i) => (
                      <Star key={i} className={`w-5 h-5 ${i < Math.round(averageRating) ? 'fill-teal-600 text-teal-600' : 'text-gray-300'}`} />
                    ))}
                  </div>
            ...
            <div className="md:col-span-2">
              <div className="space-y-4">
                {[5, 4, 3, 2, 1].map((rating) => {
                  const ratingCount = product.reviews?.reviewCountTotals.find((r) => r.rating === rating)?.count || 0;
      ...
      {product.productUrl && (
        <div className="text-center">
          <Button asChild className="bg-teal-600 hover:bg-teal-700 text-white font-medium py-3 px-8" size="lg">
            <Link href={product.productUrl} target="_blank" rel="noopener noreferrer">Book Now</Link>
          </Button>
        </div>
      )}
